...
export function getSFXNotice() { }

export function getCalypsoNotice() { }
...
export function getFirstLetterBeforeNumber(cote) {
    let myRegexp = /^([A-Z])[0-9.]/g;
    let match = myRegexp.exec(cote);
    return match ? match[1] : null
}

export function getOnlyOneLetter(cote) {
    let myRegexp = /^([A-Z])$/g;
    let match = myRegexp.exec(cote);
    return match ? match[1] : null
}
 ...
 * getNormalizedCote
 */
const getNormalizedCote = function (cote) {
...
export function hasMore3AlphaBeginingCharacters(cote) {
    let regex = /^[A-Z]{4}/g;
